...
public class ClassLoaderUtil
{
    ...
    /**
     * Create a class loader that can load classes from the specified file directory. The file directory must contains
     * .jar or .zip
     *
        ...
        throws IOException
    {
        ...
        if (libDir.exists())
        {
            if (libDir.isDirectory())
            {
                ...
                {
                    String path = new File(libDir.getName() + File.separator + jars[i]).getCanonicalFile()
                        .toURI()
                        .toURL()
                        .toString();
                    urls[i] = new URL(path);
        ...
        throws IOException
    {
        ...
        if (dirPath != null && (dirPath.endsWith(".war") || dirPath.endsWith(".jar")))
        {
            ...
            classesURL =
                new URL("jar:file:" + separator + file.getCanonicalPath().replace('\\', '/') + "!/WEB-INF/classes/");
        ...
        if (libFiles.exists() && libFiles.isDirectory())
        {
            ...
            {
                urls[i] =
                    new URL("jar:file:" + separator + libFiles.listFiles()[i].toString().replace('\\', '/') + "!/");
            }
        }
        else
        {
